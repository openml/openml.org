...
const sortOptions = [
  ...
  {
    name: "Most Tasks",
    value: [{ field: "nr_of_tasks", direction: "desc" }],
    id: "tasks",
  },
];

export function BenchmarksSearchContainer() {
                  ...
                  {view === "list" && (
                    <div className="space-y-0">
                      {results && results.length > 0 ? (
                        results.map(
                          (result: BenchmarkResult, index: number) => {
                            const benchmarkId = result.benchmark_id?.raw;
                            const name =
                              result.name?.raw || "Untitled Benchmark";
                            const description =
                              ...
                              >
                                <div className="min-w-0 flex-1">
                                  ...
                                  <div className="flex flex-wrap gap-x-4 gap-y-1 text-xs">
                                    {result.nr_of_tasks?.raw !== undefined && (
                                      <span className="flex items-center gap-1.5">
                                        <Target className="h-3 w-3 text-purple-600" />
                                        <span className="font-medium">
                                          {result.nr_of_tasks.raw.toLocaleString()}
                                        </span>
                                ...
                                <Link
                                  ...
                                  aria-label={`View benchmark ${name}`}
                                >
                        ...
                        <div className="py-12 text-center">
                          <p className="text-muted-foreground">
                            No benchmarks found. Try adjusting your search or
                            filters.
                          </p>
                        </div>
                      )}
