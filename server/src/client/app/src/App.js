...
class App extends React.Component {
    state = {
                        ...
                        if (
                            ...
                            axios
                                ...
                                .then((response) => {
                                    let img = undefined;
                                    if (
                                        response.data.image.includes(path.sep)
                                    ) {
                                    ...
                                    this.setState(
                                        ...
                                        this.log(
                                            'Login changed: User profile loaded'
                                        )
                                    );
                                })
                                .catch((error) => {
                    ...
                    .catch((error) => {
                        if (this.state.loggedIn) {
                            this.setState(
                                { loggedIn: false },
                                this.log(
                                    'Login changed: Authentication check failed'
                                )
                            );
                        }
                    });
        ...
        setUserInitials: (value) => {
            ...
            this.setState(
                { userInitials: value },
                this.log('User initials changed')
            );
        },
        ...
        setSearch: (qp, fields) => {
            ...
            if (window.innerWidth < 600) {
                ...
                if (this.state.displaySearch !== update.displaySearch)
                    idChanged = true;
            }
                ...
                <ThemeProvider theme={maTheme[this.state.currentTheme]}>
                    <StylesProvider injectFirst>
                        <MainContext.Provider value={this.state}>
                            <SCThemeProvider
                                theme={maTheme[this.state.currentTheme]}
                            >
                                <Routes />
                            </SCThemeProvider>
