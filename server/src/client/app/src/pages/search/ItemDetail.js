  ...
  render() {
    ...
    return (
      <TableRow className="contentSection item">
        ...
        <TableCell className={"itemDetail-small"}>
          {this.props.item.descriptions
            ? this.props.item.descriptions.map((obj, index) => {
...
export class EntryDetails extends React.Component {
  ...
  componentDidUpdate() {
    ...
    if (
      (this.props.type === "study" || this.props.type === "benchmark") &&
      this.state.obj !== null &&
  ...
  render() {
    ...
    if (
      (this.state.error !== null && this.state.error.includes("404")) ||
      (this.state.obj !== null &&
        this.state.obj.visibility === "private" &&
        this.state.obj.uploader_id !== this.props.userID &&
        ![1, 2, 869, 42266, 9186].includes(this.props.userID))
    ) {
      return (
        <Grid
          container
