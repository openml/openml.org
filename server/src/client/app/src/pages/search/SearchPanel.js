...
export default class SearchPanel extends React.Component {
  ...
  getQueryParams = () => {
    ...
    if (qstring.type === "data" && qstring.status === undefined) {
      ...
      qstring.type === "measure" &&
      qstring.measure_type === undefined
    ) {
      qstring.measure_type = "data_quality";
      this.updateQuery("measure_type", "data_quality");
    } else if (
      (qstring.type === "study" || qstring.type === "benchmark") &&
      qstring.study_type === undefined
    ) {
      qstring.study_type = "task";
      this.updateQuery("study_type", "task");
    } else if (
      (qstring.type === "study" || qstring.type === "benchmark") &&
      this.context.filters.study_type !== undefined &&
      qstring.study_type !== this.context.filters.study_type.value
    ) {
    ...
    if (
      qstring.sort === undefined &&
      (this.context.sort === null ||
        this.context.sort === undefined ||
        this.context.type !== qstring.type)
    ) {
  ...
  shouldComponentUpdate(nextProps, nextState) {
    return (
      this.context.updateType !== "query" || this.context.results.length >= 0
    );
  }
        ...
        if (key.startsWith("tags")) {
          filters[key].value.split("-").forEach((t) => {
            queryFilters.push({
              nested: {
                path: "tags",
                query: { term: { "tags.tag": t.toLowerCase() } },
              },
            });
          });
          ...
          queryFilters.push({
            nested: {
              path: "collections",
              query: { term: { "collections.id": filters[key].value } },
            },
          });
  ...
  processSearchResults = (data, type) => {
    ...
    if (type === "task") {
      data.results.forEach((res) => {
        res["comp_name"] =
          getProperty(res, "source_data.name") +
          " " +
        ...
        if (getProperty(res, "tasktype.name").includes("Supervised")) {
          res["description"] =
            "Predict feature '" + getProperty(res, "target_feature") + "'. ";
          if (
            getProperty(res, "target_values") &&
            getProperty(res, "target_values").length <= 10
          ) {
            res["description"] +=
              "Possible values are '" +
              getProperty(res, "target_values") +
          ...
          res["description"] =
            "Perform '" + getProperty(res, "tasktype.name") + " analysis. ";
        } else {
          res["description"] =
            "Perform " + getProperty(res, "tasktype.name") + ". ";
          if (getProperty(res, "target_value")) {
            res["description"] +=
              "The target value is '" +
              getProperty(res, "target_value") +
              "'. ";
          }
        }
        ...
        res["description"] +=
          ...
          getProperty(res, "evaluation_measures") +
          ". ";
      });
    } else if (type === "run") {
      data.results.forEach((res) => {
      ...
      data.results.forEach((res) => {
        res["comp_name"] =
          getProperty(res, "first_name") + " " + getProperty(res, "last_name");
        res["description"] =
          getProperty(res, "bio") +
          " - " +
        ...
        res["initials"] =
          getProperty(res, "first_name").charAt(0).toUpperCase() +
          getProperty(res, "last_name").charAt(0).toUpperCase();
      });
  ...
  reload() {
    ...
    search(
      ...
      .then((data) => {
        ...
        } else if (this.context.results[0][obj_id] !== res.results[0][obj_id]) {
          //add to infinite list
          this.context.setResults(
            res.counts,
            this.context.results.concat(res.results),
          );
        }
      })
      .catch((error) => {
    ...
    search(
      ...
      .then((data) => {
        ...
        if (this.context.startSubCount === 0) {
          ...
          this.context.setSubResults(
            res.counts,
            this.context.subResults.concat(res.results),
          );
        }
      })
      .catch((error) => {
  ...
  tabChange = (event, activeTab) => {
    ...
    if (
      ...
      if (
        (activeTab === 3 && this.context.subType === "task") ||
          ...
          style={{
            visibility:
              ...
              (!this.context.displayStats &&
                (this.context.id === null || this.context.id === undefined))
                ? "visible"
                : "hidden",
            height:
              this.context.displaySplit ||
            ...
            <SearchTab
              label={
                this.context.id !== undefined
                  ? this.context.type === "study" ||
                    this.context.type === "benchmark"
                    ? ucfirst(this.context.filters.study_type.value) +
                      " Collection"
