    ...
    def scatterplot_study(pathname, value, graph_type, metric):
        ...
        if value == 'mean':
            ...
            for i, (flow_name, flow_df) in enumerate(df.groupby('flow_name')):
                ...
                fold_trace = go.Scatter(x=flow_df['values'], y=y_folds, mode='markers', name=f'{flow_name}_fold', opacity=0.75, legendgroup=flow_name, showlegend=False)
                ...
                mean_trace = go.Scatter(x=flow_mean_df['values'], y=y_mean, mode='markers', marker=dict(symbol='diamond'), legendgroup=flow_name, name=flow_name)
