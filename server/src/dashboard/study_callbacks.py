...
def register_study_callbacks(app, cache):
    @app.callback(
        [Output("graph-div", "children"), Output("show-fold-checkbox", "style")],
        [
            Input("url", "pathname"),
    ...
    def scatterplot_study(
        pathname, graph_type, metric, show_fold_checkbox, shown_table_rows
    ):
        ...
        with print_duration("Callback"):
            ...
            study_results = load_run_data(
                study_id, metric, include_each_fold=show_folds, max_runs=300
            )
            ...
            dataset_link_map = {
                dataset: create_link_html(dataset, dataset_id)
            ...
            if graph_type == "parallel":
                for flow_name, flow_df in study_results.groupby("flow_name"):
                    shared_template = (
                        ...
                        "%{text}<br>"
                        "<extra></extra>"
                    )  # Removes a second box with trace info
                    ...
                    for dataset_name in dataset_names:
                        for (score, data_name) in zip(
                            flow_df["value"], flow_df["data_name"]
                        ):
                    ...
                    mean_text = [
                        (f"Dataset: {dataset_name}<br>" "Mean score")
                        for dataset_name in dataset_names
                    ]
                    fig.add_trace(
                        go.Scatter(
                ...
                fig.update_xaxes(
                    ticktext=list(dataset_link_map.values()), tickvals=dataset_names
                )
                fig.update_layout(
                    xaxis_title="Dataset",
                ...
                dataset_y_map = {
                    dataset: i for i, dataset in enumerate(reversed(dataset_names))
                }
                ...
                for i, (flow_name, flow_df) in enumerate(
                    study_results.groupby("flow_name")
                ):
                    ...
                    shared_template = (
                        ...
                        "%{text}<br>"
                        "<extra></extra>"
                    )  # Removes a 2nd box with trace info
                    ...
                    if show_folds:
                        ...
                        fold_text = [
                            f"Dataset: {dataset_name}<br>Fold score"
                            for dataset_name in data_names
                        ]
                        fold_trace = go.Scatter(
                            x=scores,
                            y=ys,
                            mode="markers",
                    ...
                    ys = [
                        dataset_y_map[dataset_name] + y_offset
                    ...
                    mean_trace = go.Scatter(
                        ...
                        y=ys,
                        mode="markers",
                        marker=dict(symbol="diamond", color=flow_color),
                        legendgroup=flow_name,
                        name=flow_name,
                        text=mean_text,
                        hovertemplate=shared_template,
                ...
                fig.update_yaxes(
                    ticktext=[
                        dataset_link_map[dataset_name] for dataset_name in dataset_y_map
                    ],
                ...
                raise ValueError(
                    f"`graph_type` must be one of 'scatter' or 'parallel', "
                    f"not {graph_type}."
                )
            ...
            fig.update_layout(
                ...
                font=dict(family="Segoe UI Symbol", size=14, color="#7f7f7f"),
            )
            ...
            checkbox_style = {
                "display": "none" if graph_type == "parallel" else "block"
            }
...
def load_run_data(study_id: int, metric: str, include_each_fold: bool, max_runs: int):
    ...
    df = openml.evaluations.list_evaluations(
        metric, runs=runs, output_format="dataframe", per_fold=include_each_fold
    )
